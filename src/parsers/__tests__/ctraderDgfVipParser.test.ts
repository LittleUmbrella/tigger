...
💣 SL 4791`;
    ...
    expect(order!.tradingPair).toBe('XAUUSD');
    expect(order!.signalType).toBe('short');
    expect(order!.entryPrice).toBe(4782);
    expect(order!.entryTargets).toEqual([4782, 4785]);
    expect(order!.marketExecution).toBe(false);
...
Use proper money management. Consistency is 🔑`;
    ...
    expect(order!.tradingPair).toBe('XAUUSD');
    expect(order!.signalType).toBe('long');
    expect(order!.entryPrice).toBe(4718);
    expect(order!.entryTargets).toEqual([4714, 4718]);
    expect(order!.marketExecution).toBe(false);
    expect(order!.stopLoss).toBe(4710);
    expect(order!.takeProfits).toEqual([4728, 4738, 4748, 4768]);
  });

  it('parses XAUUSD SELL with en-dash entry, SL =, and bare T1/T2 labels (message 14654)', () => {
    const msg =
      '🛡XAUUSD SELL 4572– 4577 💣 SL = 4580 T1 :4565 T2 : 4550 Use proper money management. Consistency is 🔑';
    const order = ctraderDgfVipParser(msg);
    expect(order).not.toBeNull();
    expect(order!.tradingPair).toBe('XAUUSD');
    expect(order!.signalType).toBe('short');
    expect(order!.marketExecution).toBe(false);
    expect(order!.entryPrice).toBe(4572);
    expect(order!.entryTargets).toEqual([4572, 4577]);
    expect(order!.stopLoss).toBe(4580);
    expect(order!.takeProfits).toEqual([4565, 4550]);
  });
  ...
  it('parses single-line XAUUSD BUY NOW with dash entry range and Tp labels', () => {
    ...
    expect(order!.signalType).toBe('long');
    expect(order!.marketExecution).toBe(false);
    expect(order!.entryPrice).toBe(4650);
    expect(order!.entryTargets).toEqual([4646, 4650]);
  ...
  it('parses single-line XAUUSD BUY with "Stop Loss:" label', () => {
    ...
    expect(order!.signalType).toBe('long');
    expect(order!.marketExecution).toBe(false);
    expect(order!.entryPrice).toBe(4805);
    expect(order!.entryTargets).toEqual([4795, 4805]);
  ...
  it('parses Format 6 single-line BTCUSD BUY with spaced dash entry and STOP LOSS (message 14564)', () => {
    ...
    expect(order!.signalType).toBe('long');
    expect(order!.marketExecution).toBe(false);
    expect(order!.entryPrice).toBe(76300);
    expect(order!.entryTargets).toEqual([76000, 76300]);
...
TP : 4800.00
 ...
 Use proper money management. Consistency is 🔑`;
  ...
  it('parses BUY@ range with mathematical italic TP labels (message 14813)', () => {
    const msg =
      '🛡XAUUSD BUY@4506~4502 𝑇𝑃1: 4520 𝑇𝑃2: 4530 💣SL 4495 Use proper money management. Consistency is 🔑';
    const order = ctraderDgfVipParser(msg);
    expect(order).not.toBeNull();
    expect(order!.tradingPair).toBe('XAUUSD');
    expect(order!.signalType).toBe('long');
    expect(order!.marketExecution).toBe(false);
    expect(order!.entryPrice).toBe(4506);
    expect(order!.entryTargets).toEqual([4502, 4506]);
    expect(order!.stopLoss).toBe(4495);
  ...
  it('parses single-line BTCUSD SELL with inconsistent TP arrow spacing (message 15302)', () => {
    ...
    expect(order!.signalType).toBe('short');
    expect(order!.marketExecution).toBe(false);
    expect(order!.entryPrice).toBe(73450);
    expect(order!.entryTargets).toEqual([73450, 73750]);
    expect(order!.stopLoss).toBe(74250);
    expect(order!.takeProfits).toEqual([72950, 72650, 72350, 72000]);
  });
